...
def new_ball():
    ...
    radius_ball = r = randrange(int(0.015*canv.winfo_width()), int(0.09*canv.winfo_height()))
    x_ball = x = randrange(r+5, canv.winfo_width()-r-5)
    y_ball = y = randrange(r+5, canv.winfo_height()-r-5)
    redius_relative = radius_ball/(0.09*canv.winfo_height() - 0.015*canv.winfo_width())
    color = choice(colors)
...
def click(event):
    ...
    if (event.x - x_ball)**2 + (event.y - y_ball)**2 <= radius_ball**2:
        ...
        canv.delete(ball)
        canv.update_idletasks()
        text_score = canv.create_text(canv.winfo_width()-10,10,text=f"{user_name}\nScore: " + str(points), font = 'Arial 20', anchor=NE)
        ...
        timer = finish_time-start_time+all_pause_time
        canv.update_idletasks()
        text_score = canv.create_text(canv.winfo_width()-10,10, text=f"{user_name}\nScore: {points}\nTime: {round(timer)} sec", font = 'Arial 20', anchor=NE)
        date_game = (datetime.date.today()).strftime("%b-%d-%Y")
        data_base()
        points = 0
        all_pause_time = 0
...
def paused(*args):
    ...
    pause_text_banner=canv.create_text(canv.winfo_width()/2, canv.winfo_height()/2, text="PAUSE", fill='red', font = 'Arial 60')
...
def pre_start(after_game=None):
    ...
    if after_game:
        ...
        cnv_frm_start = canv.create_window(logo_img_x, logo_img_y+img.height()//2+frm_start.winfo_height()/2, window=frm_start)
...
def nick(*args):
    ...
    if not pattern.match(user_name):
        messagebox.showerror("login failure", "Nickname must contain alphanumeric string max 10 symbols")
        canv.delete(cnv_frm_login)   
        login()     
    else:
        pre_start()
...
def login():
    ...
    cnv_frm_login = canv.create_window(logo_img_x, logo_img_y+img.height()//2+frm_login.winfo_height()/2, window=frm_login)
...
def data_base():
    ...
    with sqlite3.connect('database.db') as db:
        ...
        ttk.Style().configure("Treeview", background="LightSkyBlue1", fieldbackground="LightSkyBlue1", rowheight=int(0.0564*root_height))
